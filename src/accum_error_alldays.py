...
save_folder = '/www/results/verification/images/accum_error/'
...
variables = ['SFCTC_KF', 'SFCTC', 'PCPTOT', 'PCPT6', 'SFCWSPD_KF', 'SFCWSPD']
...
for i in range(len(models)):
    model = models[i]
    for grid in range(len(grids[i].split(','))):
        grid = grids[i].split(',')[grid]
        print(model) 
        if model == "ENS":
            grid = ""
...
def get_fcst(filepath, station, variable, maxhour, index_start, index_end, date_list):
    ...
    df_new = make_df(date_list, index_start, index_end)
    ...
    for x in range(len(fcst['Offset'])):
        fcst.loc[x, 'datetime'] = pd.to_datetime(index_start, format='%Y%m%d') + timedelta(hours=int(x))
...
def get_filehours(hour1,hour2):
    ...
    return(hours_list)
"""
----------------- old code from when data stored in txt files -------------------------------
...
def make_df(date_list, start_date, end_date):
...
# -------- old code for when data was stored in txt files insted of sql -------------------------------------
...
# -------------------------------------------------------------------------------------------------------------
    ...
    sql_query = "SELECT * from 'All' WHERE date BETWEEN " +str(index_start) + " AND " + str(index_end)
...
def get_data(station, variable, index_start, index_end, date_list, filepath):
    delta = 6 
    fcst = []
    for i in range(len(models)):
        ...
        for grid in grids[i].split(","): #loops through each grid size for each model
            ...
            if check_dates(index_start, delta, filepath, variable, station) == False:
...
def main(args):
    ...
    sql_path = obs_filepath + 'SFCTC' + "/3510.sqlite"
    sql_con = sqlite3.connect(sql_path)
    ...
    index_end = str(sql_result[-1]) 
    var_i = 0
    
    for var in variables:
        
        index_start = str(sql_result[0])
        date_list = listofdates(index_start)
        ...
        for station in stations:
           ...
           # add 6 to the end date
            ...
            fcst = get_data(station,var,index_start, index_end, date_list, filepath)
            
            for day in outlook_days:  

                plot(station, var, yaxis_labels[var_i], station_names[stations_i],obs,fcst,int(day),index_start)
