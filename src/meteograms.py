...
station_file = '/home/verif/verif-post-process/input/station_list_meteograms.txt'
...
def time_series(station, variable, ylabel, title, filepath, gridname):
    ...
    for i in range(len(models)):
        ...
        for grid in grids[i].split(","): #loops through each grid size for each model
            ...
            hrs, fcst,obs  = get_data(filepath, station, variable, obs)

            if fcst.isnull().values.all():
    ...
    if not os.path.exists(save_folder + start_date):
        os.makedirs(save_folder + start_date) #make this folder if it doesn't exist
...
def main(args):
    ...
    for station in stations: #loop through stations
        ...
        for var in variables: #loop through variables
            ...
            time_series(station, var, yaxis_labels[var_i], station_names[stations_i], filepath, gridname)
