...
These are the functions that feed into the sig_test.py script that calculates the statistical
signifigance of the obs vs stats. See that file for more info. 
"""
import os
import pandas as pd
import numpy as np
...
def make_textfile(model, grid, input_domain, savetype, date_entry1, date_entry2, time_domain, variable, filepath, t_test, len_fcst, numstations):
   
    if "ENS" in model:
        modelpath= model + '/'
    else:
        modelpath = model + '/' + grid + '/'
    f1 = open(textfile_folder + modelpath + input_domain + '/' + variable + '/' + "t_test_" + savetype + "_" + variable + "_" + time_domain + "_" + input_domain + ".txt",wm+"+")       
    read_f1 = np.loadtxt(textfile_folder +  modelpath + input_domain + '/' + variable + '/' + "t_test_" + savetype + "_" + variable + "_" + time_domain + "_" + input_domain + ".txt",dtype=str)  
    if date_entry1 not in read_f1 and date_entry2 not in read_f1:
        f1.write(str(date_entry1) + " " + str(date_entry2) + "   ")
        
        f1.write("%3.3f   " % (t_test))
        f1.write(len_fcst + "   ")
...
def get_statistics(delta, model,grid, input_domain, savetype, date_entry1, date_entry2, maxhour,hour,length,fcst_allstations,obs_allstations,num_stations,totalstations,time_domain,variable,filepath):
    ...
    if int(maxhour) >= hour:
        ...
        if len(fcst_rounded) == 0:
            ...
            t_test = ttest_ind(obs_rounded,fcst_rounded).pvalue
            ptint(t_test)
            ...
            make_textfile(model, grid, input_domain, savetype, date_entry1, date_entry2, time_domain, variable, filepath, t_test, len_fcst, numstations)
