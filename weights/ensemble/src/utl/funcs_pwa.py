...
def listofdates(start_date, end_date, obs = False):
    if obs == False:
        ...
        end = datetime.strptime(end_date, "%y%m%d%H")#.date() + timedelta(days=endday)
...
def mk_ensemble(stat_cat, weight_type, stat_type, model_df_name, start_date, end_date, df_all, variable):
    ...
    df3 = pd.DataFrame()
    if start_date.month in [1,2,12]:
            season = seasons[0]
    if start_date.month in [3,4,5]:
            season = seasons[1]
    if start_date.month in [6,7,8]:
            season = seasons[2]
    if start_date.month in [8,10,11]:
            season = seasons[3]
        ...
        if stat_type == 'CAT_' and 'SFCTC' not in variable:
            for m in range(len(df_all.columns)):
                ...
                f = weights_folder + "weights-seasonal/"  + stat_type + '/weights_' \
                    + stat_cat + '_' + weight_outlook + '_' + variable + '_' + season
            ...
            for m in range(len(df_all.columns)):
                ...
                f = weights_folder + "weights-seasonal/"  + stat_type + '/weights_all' \
                        + '_' + weight_outlook + '_' + variable + '_' + season
